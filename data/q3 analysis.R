...
d1 <- as.data.table(read_xlsx('C:/Users/lhongzhe/OneDrive - UGent/project/NutriBudget/T1.2 & T1.3/R code/test/rq3_database.xlsx',sheet='RQ3_database'))
...
# --- Analysis for KPI 1 -----
  ...
  d2[,sup_cat := fifelse(grepl('insect',supplement_category),'insect','other')] # the dataset was revised that microalgae is not a significant factor any more
...
# --- Analysis for KPI 2 -----
  ...
  ggplot(data=out.dcp[var!='intercept']) +
    ...
    theme(legend.position = 'bottom')+
    scale_y_discrete(limits = c("supplemental_rate","growing","finishing","whole", "rapeseed","legumes", "insect", "microalgae"))
  ...
  # make a full model with all main factors together    
  m3.full <- metafor::rma.mv(yi,vi, 
                             mods = ~supplement_category + supplemental_rate + stage,
                             data=d2,random= list(~ 1|study_ID), method="REML",sparse = TRUE)
...
# --- Analysis for KPI 3 -----
  ...
  # if you have a model with main factors AND interactions => use the "*" sign in the mods argument
  # if you have a model with ONLY interactions => use then the ":" sign
  #d2[,sup_cat := fifelse(grepl('rapeseed|grassjuice|insect',supplement_category),'other',supplement_category)] # the dataset was revised that microalgae is not a significant factor any more
