...
ggplot(data = filtered_data) +
  ...
  theme_bw() +
  geom_text(aes(x = estimate, y = var, label = significance), vjust = +0.4, hjust = -0.2, size = 4, color = "red")  +                     #Add the asterisks for significance
...
ggplot(data = filtered_data) +
  ...
  theme_bw() +
  ggtitle(         'Effect of fertilization doses on crop yield') +
  theme(
    ...
    axis.title.y = element_text(size = 12),           # Adjust y-axis title size
    axis.text.x = element_text(size = 8),             # Adjust x-axis text size
    axis.text.y = element_text(size = 7, margin = margin(t = 20, b = 10))  # Adjust y-axis text size
  )
...
filtered_data <- out.dgr %>%
  filter(factor %in% c('crop_residue', 'cover_crop', 'crop_rotation', 'tillage', 'fertilizer_type', 
                       'ph', 'bdod_mean_0_5', 'cec_mean_0_5', 'clay_mean_0_5', 'ntot_mean_0_5',
...
ordered_labels <- c(
  ...
  'pH', 'bulk density', 'cation exchange capacity', 'clay content', 'total nitrogen',
...
ggplot(data = filtered_data) +
  ...
  theme_bw() +
  geom_text(aes(x = estimate, y = label, label = significance), vjust = +0.4, hjust = -0.2, size = 4, color = "red") + 
  ggtitle('Effect of site properties on crop yield') + 
  labs(y = "var") +  # Set y-axis label to "var"
  theme(
    plot.title = element_text(size = 10, hjust = 0),  # Adjust title size and align to left
    axis.title.x = element_text(size = 12),           # Adjust x-axis title size
    axis.title.y = element_text(size = 12),           # Adjust y-axis title size
...
cols <- c('ph', 'n_dose', 'nutri_dose_P', 'nutri_dose_K',
...
ph + tmp_mean + pre_mean + pet_mean +  soc_mean_0_5 + clay_mean_0_5 + bdod_mean_0_5 + cec_mean_0_5 + phw_mean_0_5 -1,
                  data = d2, 
                  random = list(~ 1 | study_ID), 
                  method = "REML", 
                  sparse = TRUE)
...
new_column_values <- c("management_practice","management_practice", "management_practice", "management_practice","management_practice","management_practice",
                       ...
                       "soil_property", "weather_condition", "weather_condition", "weather_condition", "soil_property", "soil_property","soil_property", "soil_property", "soil_property")
...
new_column_values1 <- c("management_practice","management_practice", "management_practice", "management_practice","management_practice",
                       ...
                       "soil_property", "soil_property", "soil_property","soil_property", "soil_property", "weather_condition")
...
# Plot for management practices
ggplot(management_practices, aes(x = factor, y = estimate)) + 
  geom_bar(stat = "identity", aes(fill = colorfactor1), alpha = 0.7, fill = "lightblue") + 
  geom_text(aes(label = significance, y = ifelse(estimate > 0, estimate + 0.001, estimate - 0.001)), 
            vjust = ifelse(management_practices$estimate > 0, -0.5, 1.5), size = 3.5) + 
  theme_minimal() + 
  labs(x = 'Crop, soil and fertilisation practices', y = 'Parameter estimate', 
