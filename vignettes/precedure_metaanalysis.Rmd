...
```{r meta regression helper, eval = FALSE}
...
```{r prepare global raster maps, eval = FALSE}
...
```{r application meta-regression, eval = FALSE}
...
Below a simple R code to plot a spatial map with a raster file on top of the polygon.

```{r plot world map,eval=FALSE}
  ...
  p1 <- ggplot(data = world) + geom_sf(color = "black", fill = "gray92") +
      ...
      coord_sf(crs = 4326)
  # save the file (use arguments width, height and units to adapt the size of the plot on disk)
